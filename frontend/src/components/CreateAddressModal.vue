  ...
  >
    <v-card>
      <v-form v-model="address.formModel" ref="formRef">
        <v-card-title>
          <h1 class="text-h5">Address creation</h1>
        </v-card-title>
...
<script setup lang="ts">
  ...
  const submit = async () => {
    ...
    if (!isLoadingCreateAddress.value && isSuccessCreateAddress.value) {
      resetAndCloseModal();
  ...
  const closeModal = () => {
    setConfirmationDataAndShow({
      title: "Address creation cancellation",
      content: "Do you really want to cancel the creation of the address? " +
